...
\begin{definition}
  ...
  A \emph{global sample\/} is a maximally consistent set of formulas of the form $\sbar\phi(x^{\rm s})\approx_\epsilon\mu$, where $\phi(x)\in L(\U)$, and $\epsilon,\mu\in\RR$.\QED
\end{definition}
...
\begin{definition}\label{def_} 
  Let $M$ be given. A global sample $p(x^{\rm s})$ is
  \begin{itemize}
    \item[1.]\noindent\emph{invariant\/} if $\sbar\phi(p\,;b)\approx\sbar\phi(p\,;b')$ for every $b\equiv_{M^{\rm f}}b'$ and every  $\phi(x\,;z)\in L$.
    \item[2.]\noindent\emph{finitely satisfiable\/} if for every finite subset of $p$ is realized in  $M^{\rm f}$.
