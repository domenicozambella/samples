...
%
It will be defined in the next section as it is not relevat for at present. 
...
\begin{notation}
  ...
  (the same notation applies with $x$ for $x^{\rm s}$). This notation intentionally confuses $p$ with any of its realizations in some elementary extension of $\barU$.
  %
...
\begin{proof}
  ...
  Now, let $\psi(x^{\rm s}\,;z)\in \barL(\barM)$. As $\psi(p\,;\U)=\psi(s\,;\U)$ is definable in $\barU^*$.
  %
...
\begin{lemma}
  ...
  \begin{itemize}
    ...
    \item[2.] $p$ is definable and for every $\phi(x^{\rm s}\,;z)\in L$ there are some finitely many $r_i\in \barM$, say $0\le i<n$, such that for every $b\in\U^{|z|}$ we have $\phi(p\,;b)\iff\phi(r_i\,;b)$ for some $i$;
...
\begin{proof}
  ...
  %
  Hence compatness yields the required $r_i\in\barM$.
  ...
  %
  For any $\phi(x^{\rm s}\,;z)\in \barL(\barM)$ and $r\in M^{\rm s}$ there is a formula $\theta(z)\in L(M)$ defining the set $\{b\in\U^{|z|}:\phi(p\,;b)\iff\phi(r\,;b)\}$.
  %
  Let $q(x^{\rm s})$ be the type containing all the formulas

  \ceq{\,}{\,}{\A z\Big[\theta(z)\wedge\phi(r\,;z)\imp\phi(x^{\rm s};z)\Big].}

  as $\phi(x^{\rm s}\,;z)$ and $r$ range as above.
  %
  This type is consistent and is equivalent to $p(x^{\rm s})$.  
  %
