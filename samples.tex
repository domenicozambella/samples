...
\begin{definition}\label{def_uniform_sample}
  ...
  %
  We say that $s$ is \emph{uniform\/} on $\B$ for $\phi(x\,;z)$ over $M$ if for every $\epsilon$ there is a finite sample $s_0\in M^{\rm f}$ such that $\Av_s\phi(x,b)\approx_\epsilon\Av_{s_0}\phi(x,b)$ for every $b\in\B$. Reference to $\phi(x\,;z)$ and $M$ will be omitted when clear from the context.\QED
\end{definition}


\begin{lemma}\label{lem_cons}
  Let $s$ be uniform on $\B$ for $\phi(x\,;z)$ over $M$ and such that $\Av_s\phi(x\,;b)>\epsilon$ for every $b\in\B$. Then there is a finite cover of $\B$, say $\B_1,\dots,\B_n$, such that all types $p_i(x)=\{\phi(x\,;b):b\in\B_i\}$ are consistent.
\end{lemma}
