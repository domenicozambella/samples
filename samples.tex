...
Al posto di $\RR$ si potrebbe usare $\QQ$ o anche meglio $\omega$.

Per campioni si potrebbero prendere anche sequenze.

Per semplicit\`a di notazione conviene usare misure con segno (e quindi campioni con segno). 
Ma non so quale sia il modo giusto. 
Per il momento faccio come se il problema non esistesse.

Soprattutto manca un'operazione tra campioni.
Cioè se $M$ è un semigruppo, vorrei avere un concetto di moltiplicazione tra campioni.
...
%
The \emph{finite (fractional) sample expansion\/} of $M$ is a therefore-sorted expansion $\bar M=\<M,\RR,M^{\rm s}\>$ that has a domain for $M$, which we call the home-sort, a domain for $\RR$, as an ordered field, and a domain $M^{\rm s}$, which we call \emph{sample-sort,} which is an  $\RR$-vector space that we describe below.
%
Variables and elements of sample-sort (or tuple thereof) are denoted with symbols $x^{\rm s}, a^{\rm s}$ and variations thereof.
%
The variable $x$ is reserved for the home-sort.
%
Otherwise we the context will clarify the sort.

An element $a^{\rm s}\in M^{\rm s}$ is a map $a^{\rm s}:M\to\RR$ which is, non negative and almost always zero.
%
These functions, which we call samples, are interpreted as (weighted) samples or as (signed) finite measures (up to normalization) that concentrated on the support of $a^s$.

%We write \emph{$\bar M$\/} for the three sorted expansion above.
%
The language of the expansion $\bar M$ is denoted by \emph{$L$.}
%
It expands the natural language of each sort: the language of $M$, the language ordered rings, and the language of $\RR$-vector spaces.
%
Moreover, for every formula $\phi(x\,; y)\in L$, where $y$ is a tuple a mixed sort, there is a symbol $\sbar\phi(x^{\rm s}\,;y)$ for a function $(M^s)^x\times M^{y}\to\RR$.
%
For $\phi(x)\in L(\bar M)$, we interpret $\sbar\phi(x^{\rm s})$ as the function that maps $a^{\rm s}\in M^{\rm s}$ to\medskip

\ceq{\hfill\emph{$\sbar\phi(a^{\rm s})$} }
{=}
{\raisebox{0ex}{$\displaystyle\sum_{x\in\phi(M)}\kern-.8ex a^s(x)$}
  \kern-1.2ex\raisebox{-2ex}{$\Bigg/$}\kern-1ex
  \raisebox{-2ex}{$\displaystyle\sum_{x\in M} a^s(x)$}
}\medskip
...
\begin{definition}
  ...
  %
  Let \emph{$\bar\U=\<\U,\RR,\U^{\rm s}\>$\/} be as above and let \emph{$\bar\U^*$} be a saturated elementary extension of $\U^{\rm s}$ of cardinality $\kappa$.
  As all saturated models of cardinality $\kappa$ are isomorphic, we can assume that $\U$ is the domain of the home-sort of $\bar\U^*$, hence we write $\bar\U^*=\<\U,\RR^*,\U^{\rm s*}\>$.  
  %
  Elements of $\U^{\rm s}$ are called \emph{finite samples,} elements of $\U^{\rm s*}$ are called \emph{internal samples.}

  %Finally, a \emph{external sample\/} is a maximally consistent set of formulas of the form $\sbar\phi(x^{\rm s})\approx_\epsilon\mu$, where $\phi(x)\in L(\U^{\rm s*})$, and $\epsilon,\mu\in\RR$.
  
  Finally, an \emph{external sample\/} is a maximally consistent set of formulas $p(x^{\rm s})\subseteq L(\U,\RR,\U^{\rm s*})$. 
...
\begin{definition}\label{def_} 
  ...
  \begin{itemize}
    \item[1.]\noindent\emph{invariant\/} if $\phi(p\,;\U,\U^{\rm s*})$ is invariant over $\bar M$, for every $\phi(x\,;y,z^{\rm s})\in L$.
    \item[2.]\noindent\emph{finitely satisfiable\/} if every formula in $p(x^{\rm s})$ is satisfied by some element of $M^{\rm s}$.
    \item[3.]\noindent\emph{definable\/} if $\phi(p\,;\U,\U^{\rm s*})$ is definable over $\bar M$, for every $\phi(x\,;y,z^{\rm s})\in L$.\QED
  \end{itemize}
...
% For every $a^{\rm s}\in\U^{\rm s*}$ and $b^{\rm s}\in\U^{\rm s*}$ we define
% %
% \ceq{\hfill\emph{$a^{\rm s}\cnonfork_Mb^{\rm s}$}}
% {\IFF}
% {\phi(\U^{\rm s*},b^{\rm s})\cap M^{\rm s}\neq\0
% \textrm{ for all }\phi(x^{\rm s}\,;z^{\rm s})\in L(M,\RR,M^{\rm s}) 
% \textrm{ such that }\phi(a^{\rm s}\,;b^{\rm s})}.
...
\begin{fact}\label{fact_smooth=>fs}
  Let $M$ be given. Every smooth invariant sample is finitely satisfiable and definable.
\end{fact}

\begin{proof}
  Let $s$ be an internal sample that realizes an invariant external sample $p(x^{\rm s})$.
  %
