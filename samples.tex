...
%
Variables and elements of sample-sort (or tuple thereof) are denoted with symbols $x^{\sf s}, a^{\sf s}$ and variations.
%
The variable $x$ is reserved for the home-sort, for all other variables we the context will clarify the sort.

An element $a^{\sf s}\in M^{\sf s}$ is a map $a^{\sf s}:M\to\RR$ which is almost always zero.
%
These functions, which we call samples, are interpreted as (weighted) samples or as (signed) finite measures (up to normalization) that concentrated on the support of $a^{\sf s}$.

The language of the expansion $\bar M$ is denoted by \emph{$L$.}
%
It expands the natural language of each sort: on $M$, the language of semigroups, on $\RR$, the language ordered rings, and, on $M^{\sf s}$, the language of algebras over $\RR$. 
This algebra has the natural sum and the multiplication inherited from $M$

\ceq{\hfill (a^{\sf s} + b^{\sf s}) (z)}
{=}
{a^{\sf s}(z) + b^{\sf s} (z)};\smallskip

\ceq{\hfill (a^{\sf s}\cdot b^{\sf s}) (z)}
{=}
{\sum_{x\cdot y=z}a^{\sf s}(x)\cdot b^{\sf s} (y)}.

Finally, for every formula $\phi(x\,; y)\in L$, where $y$ is a tuple a mixed sort, there is a symbol $\sbar\phi(x^{\sf s}\,;y)$ for a function $(M^s)^x\times M^{y}\to\RR$.
%
For $\phi(x)\in L(\bar M)$, we interpret $\sbar\phi(x^{\sf s})$ as the function that maps $a^{\sf s}\in M^{\sf s}$ to\medskip

\ceq{\hfill\emph{$\sbar\phi(a^{\sf s})$} }
{=}
...
\begin{definition}
  ...
  %Finally, a \emph{external sample\/} is a maximally consistent set of formulas of the form $\sbar\phi(x^{\sf s})\approx_\epsilon\mu$, where $\phi(x)\in L(\U^{\sf s*})$, and $\epsilon,\mu\in\RR$.
...
\section{Semigroup operation among samples}

We work over a model $M$.
We write $a_{\bar M}$ for the orbit of $a$ over $\bar M$.
...
In this section we assume that for every $\phi(x),\psi(x)\in L(\U)$ if $\phi(x)\imp\psi(x)$ then $\sbar\phi(x^{\sf s})\le\sbar\psi(x^{\sf s})$.
...
\begin{proof} Let $s$ be an internal sample that realizes $p(x^{\sf s})$.
  ...
  \ceq{\hfill p(x^{\sf s})}{=}{\Big\{\sbar\phi(x^{\sf s}\,;b)\approx_\epsilon\mu_b\ :\ b\in\U^{|z|}\Big\}}
