...
As all saturated models of cardinality $\kappa$ are isomorphic, we can assume that $\U$ is the home-sort of $\U^{\rm f*}$.
 ...
 We write \emph{$\supp(s)$\/} for the \emph{support\/} of $s$, that is, the subset of $\U^n$ where $x\in s$ is positive.
 %
 Note that $\supp(s)$ is a definable subset of $\U^{\rm f*}$, hence its cardinality is either finite or $\kappa$.
 %
 Moreover, $s\subseteq M^n$ for some small model $M$, then $\supp(s)$ is finite.
...
\begin{lemma}
  For every formula $\phi(x\,;z)\in L$, every sample $s\subseteq\U^{|x|}$ and every infinite $B\subseteq\U^{|z|}$ such that $\Av_s\phi(x\,;b)\not\approx0$ for every $b\in B$, there is a finite cover of $B$, say $B_1,\dots, B_n$, such that all types $\{\phi(x\,;b):b\in B_i\}$ are consistent.
\end{lemma}

\begin{proof}
  Let $M^{\rm f*}\preceq\U^{\rm f*}$ be sufficiently saturated and such that $B\subseteq M^{|z|}$.
  %
  Define  for every $\epsilon$
...
By saturation, there is an $s'\in M^{\rm f*}$ that realises the union of all these types.
%
Then $\Av_{s'}\phi(x\,;b)\not\approx0$ for every $b\in B$.
%
As $s'\subseteq M^{|x|}$ then $\supp(s')$ is finite.
%
Clearly, we can assume that $a\in s'$ is a standard number for all $a$.
%
...
\begin{corollary}
  For every formula $\phi(x\,;z)\in L$, every sample $s\subseteq\U^{|x|}$ and every sequence of indiscernibles $\<b_i:i<\omega\>$ such that $\Av_s\phi(x\,;b_i)\not\approx0$ for every $i<\omega$, the type $\{\phi(x\,;b_i):i<\omega\}$ is consistent.\QED
\end{corollary}
...
\begin{lemma}
  Let $\phi(x\,;z)\in L$ be a nip formula and let $s\subseteq\U^{|x|}$ be asample.
  %
  There is no sequence $\<b_i:i<\omega\>$ of indiscernibles such that $\Av_s\big[\phi(x\,;b_i)\niff\phi(x\,;b_j)\big]\not\approx 0$ for all $i<j<\omega$.
\end{lemma}
