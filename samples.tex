...
%
The \emph{multiset expansion\/} of $M$ is a many-sorted expansion that has a sort for $M$, which we call the home-sort, a sort for $\RR$, and a \emph{sample-sort\/} for the set of functions $s:M\to\RR$ that are almot always $0$.
%
...
When $\phi(x)$ is the formula $x=x$ we write $|x|$ for $\sbar\phi(x^{\rm s})$.
...
\begin{definition}\label{def_smooth_global}
  Let $M$ be given.
  %
  A global sample is \emph{smooth\/} if it is invariant and realized in $\barU^*$.\QED
\end{definition}
...
\begin{definition}\label{def_ssf}
  Let $M$ be given.
  %
  We say that the global sample $p(x^{\rm s})$ is \emph{strongly finitely satisfiable\/} if for every $\phi(x^{\rm s};z)\in\barL(\barM)$ there is a finite $R\subseteq M^{\rm s}$ such that for every $b\in\U^{|z|}$ we have $p\proves\phi(x^{\rm s};b)\iff\phi(r\,;b)$ for some $r\in R$.\QED
\end{definition}
...
\begin{lemma}\label{lem_sfs}
  ...
  \begin{itemize}
    ...
    \item[2.] $p(x^{\rm s})$ is strongly finitely satisfiable.
  \end{itemize}
\end{lemma}
...
\begin{lemma}
  Let $M$ be given.
  %
  Let $p(x^{\rm s})$ be strongly finitely satisfiable.
  %
  Then for some finite $A\subseteq M^{\rm s}$ we have $p(x^{\rm s})\proves\supp(x^{\rm s})=A$.
\end{lemma}

\begin{proof}
  Apply Definition~\ref{def_ssf} to the formula $x^{\rm s}(z)>0$.
  %
  Then for every $b\in\U^{|z|}$ there is an $r\in R$.

  \ceq{\hfill p(x^{\rm s})}{\proves}{x^{\rm s}(b)>0\iff r(b)>0.}
