...
%
The \emph{multiset expansion\/} of $M$ is a many-sorted expansion that has a sort for $M$, which we call the home-sort, a sort for $\RR$, and a \emph{sample-sort\/} for the set of functions $s:M\to\RR$ that have finite support (i.e.\@ are almost always $0$).
%
We write \emph{$\barM$\/}$=\<M,\RR,M^{\rm s}\>$ for the expansion above.
...
The language of $\barM$ is denoted by \emph{$\barL$.}
%
...
\ceq{\hfill\sbar\phi(x^{\rm s};z,z^{\rm s})}
{:}
{\big(M^{\rm s}\big)^{|x|}\times M^{|z|}\times\big(M^{\rm s}\big)^{|z|}\ \ \longrightarrow\ \ \RR}.

which is defined as follows

\ceq{\hfill\sbar\phi(s\,;b,r)}
{=}
{\sum\Big\{s(a)\ :\ \barM\models\phi(a\,;b,r)\Big\}}

When $\phi(x)$ is the formula $x=x$ we write $|x^{\rm s}|$ for $\sbar\phi(x^{\rm s})$.


\begin{definition}
  Let $\U$ be a monster model of cardinality $\kappa>|L|$ and let $\barU=\<\U,\RR,\U^{\rm s}\>$ be the corresponding expansion.
  %
  We denote by \emph{$\barU^*$\/} a saturated elementary extension of $\barU$ of cardinality $\kappa$.
  %
  As all saturated models of cardinality $\kappa$ are isomorphic, we can assume that $\barU^*=\<\U,\,\RR^*,\U^{\rm s*}\>$, where $\RR^*$ and $\U^{\rm s*}$ are suitable saturated extensions of $\RR$, respectively $\U^{\rm s}$.
  %
  Elements $\U^{\rm s}$ are called \emph{finite samples,} elements in $\U^{\rm s*}$ are called \emph{internal samples.}

  Finally, a \emph{global sample\/} is a type $p(x^{\rm s})$ that is complete among formulas without quantifiers of sample-sort and with parameters in $\U,\barM$, where $M$ is a model which will be clear from the context.\QED
\end{definition}


\begin{notation}
  For any global type $p(x^{\rm s})\in S(\U,\barM)$, formula $\phi(x^{\rm s};z)\in\barL(\barM)$, and $b\in\U^{|z|}$ we write \emph{$\phi(p\,;b)$} for $\phi(x^{\rm s}; b)\in p$.
  %
  We also write

  \ceq{\hfill\emph{$\phi(p\,;\U)$}}{=}{\Big\{ b\in\U^{|z|}\ :\ \phi(p\,;b)\Big\}}

  Sets of this form are called \emph{externally definable.}
  %
  This notation intentionally confuses $p$ with any of its realizations in some elementary extension of $\barU$.\QED
\end{notation}
...
\begin{definition}\label{def_}
  ...
  \begin{itemize}
    \item[1.]\noindent\emph{invariant\/} if for every $\phi(x^{\rm s};z)\in\barL(\barM)$ the set $\phi(p\,;\U)$ is invariant over $\barM$;
    \item[2.]\noindent\emph{finitely satisfiable\/} if for every $\phi(x^{\rm s})\in p$  there is an $r\in M^{\rm s}$ such that $\phi(p)\iff\phi(r)$;
    \item[3.]\noindent\emph{definable\/} if for every $\phi(x^{\rm s};z)\in\barL(\barM)$ the set $\phi(p\,;\U)$ is definable over $\barM$.\QED
  \end{itemize}
\end{definition}
...
\begin{definition}\label{def_smooth_global}
  Let $M$ be given.
  %
  A global sample is \emph{smooth\/} if it is invariant and internal (i.e.\@ realized in $\barU^*$).\QED
\end{definition}

We recall the following evident fact.

\begin{fact}
  Let $M$ be given.
  %
  Let $p(x^{\rm s})$ be a finitely satisfiable global sample.
  %
  Then $p$ is invariant.\QED
\end{fact}

%Note that a global sample $p(x^{\rm s})$ is finitely satisfiable if for every finite many $b_i\subseteq\U^{|z|}$ there is an $r\in M^{\rm s}$ such that $\phi(p\,;b_i)\iff\phi(r\,;b_i)$ for all $i$.

\begin{definition}\label{def_ssf}
  Let $M$ be given.
  %
  A global sample $p(x^{\rm s})$ is \emph{uniformly (finitely) satisfiable\/} if for every $\phi(x^{\rm s};z)\in\barL(\barM)$ there is a finite $R\subseteq M^{\rm s}$ such that for every $b\in\U^{|z|}$ we have $p\proves\phi(x^{\rm s};b)\iff\phi(r\,;b)$ for some $r\in R$.\QED
\end{definition}

Let  $p(x^{\rm s})$ be a global sample. The support of $p(x^{\rm s})$  is the set of those $a\subseteq\U$ such that $p(x^{\rm s})\proves x^{\rm s}(a)\neq0$.

\begin{lemma}\label{lem_finsupp}
  Let $M$ be given.
  %
  The support of a uniformly satisfiable samples is a finite subset of $M$.
\end{lemma}

\begin{proof}
  Apply Definition~\ref{def_ssf} to the formula $x^{\rm s}(z)\neq0$.
  %
  Then for every $b\in\U^{|z|}$ there is an $r\in R$.

  \ceq{\hfill p(x^{\rm s})}{\proves}{x^{\rm s}(b)\neq0\imp r(b)\neq0.}

  As each $r\in R$ has finite support, and $R$ is finite, the lemma follows.
\end{proof}

\begin{fact}\label{fact_finite_support}
  Let $M$ be given.
  %
  Let $p(x^{\rm s})$ be a global type with finite support, say $A=\{a_1,\dots,a_n\}$.
  %
  Then  there is type $q(y)$, where $y=y_1,\dots,y_n$ is a tuple of variables of real-sort, with parameters in $\U,\barM$ and no variable of sample sort such that $p(x^{\rm s})$ is equivalrnt to

  \hfil$\E y\ \Big[ q(y)\ \cup\ \{x^{\rm s}(a_i)=y_i : i=1,\dots,n\}\Big]$
\end{fact}

\begin{lemma}\label{lem_sfs}
  Let $M$ be given.
  %
  For every smooth global sample $p(x^{\rm s})$ the following hold
  \begin{itemize}
    \item[1.] $p(x^{\rm s})$ is definable;
    \item[2.] $p(x^{\rm s})$ is uniformly finitely satisfiable.
  \end{itemize}
\end{lemma}

\begin{proof}
  Let $s\in\barU^{\rm s*}$ be an internal sample that realizes $p(x^{\rm s})$.
  %
  Let $\phi(x^{\rm s};z)\in L$ be given.
  \begin{itemize}
    \item[1.] As $\phi(p\,;\U)=\phi(s\,;\U)$ is definable in $\barU^*$, 
    by invariance it is definable in $\barM$.
    \item[2.] First we prove that $p(x^{\rm s})$ is finitely satisfiable.
    %
    Let $b\in\U^{|z|}$ be given and pick $b'\equiv_{\barM}b$ be such that $s\cnonfork_{\barM}b'$.
    %
    ...
    \ceq{\hfill q(z)}{=}{\bigg\{\phi(s\,;z)\iff\phi(r\,;z)\ :\ r\in M^{\rm s}\bigg\}}

    Hence compatness yields the required $R\subseteq M^{\rm s}$.\qedhere
  \end{itemize}
\end{proof}
...
\begin{lemma}
  Let $M$ be given.
  %
  Let $p(x^{\rm s})$ be a global type, then the following are equivalent
  \begin{itemize}
    \item[1.] $p(x^{\rm s})$ is smooth;
    \item[2.] $p(x^{\rm s})$ is uniformly finitely satisfiable.
  \end{itemize}
\end{lemma}
...
\begin{proof}
  ...
  %
  By Lemma~\ref{lem_finsupp},  $p(x^{\rm s})$ has a finite support $A\subseteq M$.   
  %
  Now apply Fact~\ref{fact_finite_support}.
  %
  As the type $q(y)$ is relized in $\barU^*$, so is  $p(x^{\rm s})$.
  %
...
\begin{lemma}
  ...
  %
  Then for every $\epsilon$ there are finitely many formulas $\theta_i(x)\in L(M)$ such that for every $b\in\U^{|z|}$ we have $\theta_i(x)\imp\phi(x\,;b)\imp\theta_j(x)$ and $\sbar\theta_i(p)\approx_\epsilon\sbar\theta_j(p)$ for some $i,j$.
\end{lemma}
...
% We assume that for every $\phi(x),\phi(x)\in\barL(\barU)$ if $\phi(x)\imp\phi(x)$ then $\sbar\phi(x^{\rm s})\le\sbar\phi(x^{\rm s})$.
...
\begin{proof}
  ...
  %
  We claim that there is an $r\in \barM$ such that $\sbar\phi_i(p)\approx_{3\epsilon}\sbar\phi_i(r)$ for every $i$.
  ...
  %
  By elementarity there is an $r\in \barM$ such that $\theta_{i,1}(r)\approx_\epsilon\theta_{i,1}(p)$ and $\theta_{i,2}(r)\approx_\epsilon\theta_{i,2}(p)$.
  %
...
%Clearly, if $s$ is an internal sample, then $p(x^{\rm s})=\tp(s/\barU)$ is a smooth sample w.r.t.\@ any given formula $\phi(x\,;z)\in L$.
...
\begin{definition}\label{def_genstable}
  ...
  A global sample $p(x^{\rm s})$ is
  \begin{itemize}
    \item[1.]\noindent \emph{invariant\/} if $\bar\phi(p\,;b)\approx\bar\phi(p\,;b')$ for every $b\equiv_{\barM}b'$;
    \item[2.]\noindent (pointwise) \emph{approximable\/} if, for every $b\in\U^{|z|}$ and every $\epsilon$, there is an $r\in \barM$ such that $\sbar\phi(p\,;b)\approx_\epsilon\sbar\phi(r\,;b)$;
    \item[3.]\noindent \emph{definable\/} if, for every $\epsilon$, the set $\{b\in\U^{|z|}:\sbar\phi(p\,;b)<\epsilon\}$ is definable over $\barM$.\QED
  \end{itemize}
\end{definition}
...
%
If $s$ is a smooth sample  \emph{$\supp(s)$\/} is defined to be  \emph{$\supp(p)$\/} for $p(x^{\rm s})=\tp(s/\barU)$.
...
\begin{definition}\label{def_genstable}
  ...
  \begin{itemize}
    ...
    \item[2.] \noindent \emph{definable\/} if, for every $\epsilon$, the set $\{b\in\U^{|z|}:\sbar\phi(s\,;b)<\epsilon\}$ is definable over $M$;
          % \item [3.]\noindent  \emph{satisfiable\/} if, for all $b\in\U^{|z|}$, if $\sbar\phi(s\,;b)>\epsilon$, then  $\phi(r\,;b)>\epsilon$ for some $r\in \barM$.
    \item [3.]\noindent  \emph{finitely satisfiable\/} if, for all $b\in\U^{|z|}$ there is an $r\in \barM$ such that  $\sbar\phi(s\,;b)\approx_\epsilon\sbar\phi(r\,;b)$;
