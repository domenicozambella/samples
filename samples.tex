...
These functions will be interpreted as \emph{(signed and fractional) multisets}.
%
...
\begin{definition} Let $\phi(x\,;z)\in L$ and $M$ be given. We say that a sample $s$ is \emph{smooth\/} if, for every $s'\equiv_Ms$ and every $b\in\U^{|z|}$, we have $\sbar\phi(s';b)\approx\sbar\phi(s\,;b)$. When $\phi(x\,;z)$ and $M$ are not clear from the context we will say \emph{smooth for $\phi(x\,;z)$ over $M$.}\QED
\end{definition}
...
\begin{definition}
 ...
 An external sample $s$ is 
  \begin{itemize}
    \item[1.]\noindent \emph{invariant\/} if, for every $b'\equiv_Mb\in\U^{|z|}$, we have $\sbar\phi(s\,;b)\approx\sbar\phi(s\,;b')$.
    \item[2.] \noindent \emph{definable\/} if, for every $\epsilon$, the set $\{b\in\U^{|z|}:\sbar\phi(s\,;b)<\epsilon\}$ is definable over $M$.
    \item [3.]\noindent  \emph{satisfiable\/} if, for all $b\in\U^{|z|}$, if $\sbar\phi(s\,;b)\not\approx0$, then  $\phi(a\,;b)$ for some $a\in M^{|z|}$.
    \item[4.] \noindent  \emph{generically stable\/} if all of the above holds.\QED 
  \end{itemize}
...
\begin{fact}
  Smooth samples are generically stable.
\end{fact}

\begin{proof}
  We prove in turn \ssf{1}-\ssf{3} of the definition above.
  
  \ssf{1.} If $s$ is smooth for $\phi(x\,;z)$ over $M$, then there is an internal sample $s'\in\U^{\rm f_*}$ such that $\sbar\phi(s';b)\approx\sbar\phi(s\,;b)$ for all $b\in\U^{|z|}$.
  %
  Then \ssf{1} follows.
  
  \ssf{2.} The set $\{b\in\U^{|z|}:\sbar\phi(s\,;b)<\epsilon\}$ is definable in $\U^{\rm f_*}$ because, by invariance of $s$, we may replace $s$ with any internal sample $s'\equiv_Ms$.
  %
  ...
  %
  Let $s_0\in M^{\rm f}$ such that $\sbar\phi(s_0\,;b)>\epsilon$.
  %
...
\begin{proof}
  ...
  %  
  Assume for a contradiction that $\sbar\phi(s';b)\not\approx_\epsilon\sbar\phi(s\,;b)$ for some $b\in\U^{|x|}$ and some sample $s'\equiv_M s$.
  %
...
\begin{lemma}
  ...
  \begin{itemize}
    \item[1.] $s$ is smooth;
    \item[2.] for every $b\in\U^{|z|}$ there is a finite sample $s_0\in M^{\rm f}$ such that $\sbar\phi(s\,;b)\approx_\epsilon\sbar\phi(s_0\,;b)$. (The sample $s_0$ depends on $M$, $\epsilon$ and $b$.)
    \item[3.] ????? there are some finite samples $s_1,\dots,s_n\in M^{\rm f}$ such that for every $b\in\U^{|z|}$ we have $\sbar\phi(s\,;b)\approx_\epsilon\sbar\phi(s_i\,;b)$ for some $s_i$. (The samples $s_1,\dots,s_n$ as well as the number $n$ depend on $\epsilon$ and $M$.)
  \end{itemize}
\end{lemma}
...
\begin{proof}
  ...
  %
  As $s'\cnonfork_{M^{\rm f}}b$ there is an $s_0\in M^{\rm f}$ such that $\sbar\phi(s_0\,;b )\approx_\epsilon\mu$.

  We prove \ssf{2}$\IMP$\ssf{3} by compactness.
  %
  Suppose not, then the following global type is finitely consistent, hence realized in $\U^{\rm f^+_*}$

  This contradicts \ssf{2}.
\end{proof}
...
\begin{definition}\label{def_approximable_sample}
  ...
  %
  We say that $s$ is \emph{approximable\/} on $\B$ if for every $\epsilon$ there is a finite sample $s_0\in M^{\rm f}$ such that $\sbar\phi(s,b)\approx_\epsilon\sbar\phi(s_0,b)$ for every $b\in\B$. (The sample $s_0$ depends on $\epsilon$ and $M$.) We add \emph{for $\phi(x\,;z)$ over $M$\/} when these are not clear from the context.\QED
\end{definition}

\begin{lemma}\label{lem_cons}
  Let $\phi(x\,;z)\in L$ and $M$ be given.
  %
  Let $s$ be approximable on $\B$ and such that $\sbar\phi(s\,;b)>\epsilon$ for every $b\in\B$. Then there is a finite cover of $\B$, say $\B_1,\dots,\B_n$, such that all types $p_i(x)=\{\phi(x\,;b):b\in\B_i\}$ are consistent.
\end{lemma}

\begin{proof}
  Let $s_0\in M^{\rm f}$ be as in Definition~\ref{def_approximable_sample}.
  %
  As $s_0$ is finite, we may assume that $\supp(s_0)=\{a_1,\dots,a_n\}$.
  %
  Let $\B_i=\{b\in\B:\phi(a_i\,;b)\}$.
  % 
  As $\sbar\phi(s_0\,;b)>0$ for all $b\in\B$, these $\B_i$ are the required cover of $\B$.
\end{proof}
...
\begin{proof}
  By Theorem~\ref{thm_VC}, there is an $s_0\in M^{\rm f}$ such that $\sbar\phi(s\,;b)\approx_\epsilon\sbar\phi(s_0\,;b)$ for all $b\in M^{|z|}$.
  
  Suppose for a contradiction that $\sbar\phi(s\,;b')\not\approx_\epsilon\sbar\phi(s_0\,;b')$ for some $b'\in\U^{|z|}$.
  %
  Pick a  $s'\subseteq\U^{|x|}$ such that $b'\cnonfork_{M}s'\equiv_Ms$.
  %
  By the smoothness of $s$, we obtain $\sbar\phi(s';b')\not\approx_\epsilon\sbar\phi(s_0\,;b')$.
  %
