...
Let $M$ be a structure of signature $L$. The \emph{finite (fractional) sample expansion\/} of $M$ is a many-sorted expansion that has a sort for $M$, which we call the home-sort, a sort for $\RR$, the set of real numbers, and for each $n$ a sort for $F_n$, an $\RR$-vector space of functions $f:M^n\to\RR$. We refer collectively to these vector spaces as \emph{sample-sort.} Variabels (or tuples of variables) of sample-sort are denoted with sumbolds $x^{\rm s}, y^{\rm s}$ and variations thereof.   
...
%
It expands $L$, the language of $\RR$ as an ordered rings, and the language of vector spaces over $\RR$.
%
...
\begin{definition}
  ...
  %
  Elements $r\in\U^{\rm f}$ are called \emph{finite samples,} elements in $s\in\U^{\rm s}$ are called  \emph{smooth samples.}\QED
\end{definition}
...
\begin{notation}
  ...
  %
  If $\phi(x\,;z)\in L$ and $p(x^{\rm s})$ is a global sample, we write $\sbar\phi(p\,;b)$ for the unique $\mu\in\RR$ such that $p(x^{\rm s})\proves \bar\phi(x^{\rm s}\,;b)\approx\mu$.\QED
\end{notation}

\begin{definition}\label{def_smooth_global}
  Let $\phi(x\,;z)\in L$. We say that the global type $p(x^{\rm s})$ is \emph{smooth\/} if there is a smooth sample $s$ such that $\sbar\phi(s\,;b)\approx\sbar\phi(p\,;b)$ for all $b\in\U^{|z|}$.\QED
\end{definition}
...
\begin{definition}\label{def_genstable}
  ...
  \begin{itemize}
    \item[1.]\noindent \emph{invariant\/} if $\bar\phi(p\,;z)$ is invariant under $\Aut(\U/M)$, that is, $\bar\phi(p\,;b)\approx\bar\phi(p\,;fb)$ for every $b\in\U^{|z|}$ and $f\in\Aut(\U/M)$;
    \item[2.] \noindent \emph{definable\/} if, for every $\epsilon$, the set $\{b\in\U^{|z|}:\sbar\phi(p\,;b)<\epsilon\}$ is definable over $M$;
    \item [3.]\noindent  \emph{finitely satisfiable\/} if, for all $b\in\U^{|z|}$ there is an $r\in M^{\rm f}$ such that  $\sbar\phi(p\,;b)\approx_\epsilon\sbar\phi(r\,;b)$;
...
\begin{proof}
  ...
  \ssf{2.} Note that the set $\{b\in\U^{|z|}:\sbar\phi(s\,;b)<\epsilon\}$ is definable in $\U^{\rm s}$. In fact, by the smoothness of $s$, we may replace $s$ with any internal sample $s'\equiv_Ms$.
  %
