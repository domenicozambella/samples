...
%
It expands $L$, the language of rings with a constat for every real number, and the language of vector spaces over $\RR$.
%
...
\begin{proof}
  ...
  %
  Suppose for a contradiction that there is an $s'\equiv_Ms$ busuch that $\sbar\phi(s'\,;b)\not\approx_\epsilon\sbar\phi(s\,;b)$.
  %
...
\begin{proof}
  ...
  \ssf{1.} This follows easily from the fact above.
...
\begin{proof}
  ...
  The other implications are evident.
\end{proof}
...
\begin{definition}\label{def_approximable_sample}
  ...
  %
  We say that $s$ is \emph{approximable\/} on $\B$ if for every $\epsilon$ there is a finite sample $r\in M^{\rm f}$ such that $\sbar\phi(s,b)\approx_\epsilon\sbar\phi(r,b)$ for every $b\in\B$. (The sample $r$ depends on $\epsilon$.) We add \emph{for $\phi(x\,;z)$ over $M$\/} when these are not clear from the context.\QED
\end{definition}
