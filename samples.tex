...
%
 The \emph{finite (fractional) sample expansion\/} of $M$ is a many-sorted expansion that has a sort for $M$, which we call the home-sort, a sort for $\RR$, as ordered field, and for each $n$ a sort for $F_n$, an $\RR$-vector space of functions $f:M^n\to\RR$.
%
We call the sort of $F_n$ \emph{sample-sort.}
%
Variables of sample-sort are denoted with sumbolds $x^{\rm s}, y^{\rm s}$ and variations thereof.   

% These functions are interpreted as \emph{(signed and fractional) multisets}.
% %
% Namely $r(a)$ interpreted as the number of times $a$ occurs in $r$.
% %
% We use the following suggestive notation:
% \begin{itemize}
%   \item[1.] \ceq{\hfill\emph{$a\in r$}\vphantom{\sum_x} }
%   {=}
%   {|r(a)|}\hfill  this is called the  \emph{multiplicity\/} of $a$ in $r$;
%   \item[2.] \ceq{\hfill\emph{$|r|$} }
%   {=}
%   {\sum_{a\in M} a\in r}\hfill  this is called the \emph{size\/} of $r$;
%   \item[3.] 
%   \ceq{\hfill\emph{$\big|\big\{a\in r\ :\ \phi(a\,;b)\big\}\big|$}}
%   {=}
%   {\sum_{a\,\models\phi(x;b)} a\in r.}
% \end{itemize}

% We call these multisets \emph{samples\/} and refer to their sorts collectively as \emph{sample-sort}.

We write \emph{$M^{\rm f}$\/} for the expansion above.
%
The language of $M^{\rm f}$ is denoted by \emph{$L^{\rm f}$.}
%
It expands the natural laguage of each sort: $L$, the language ordered rings, and the language of $\RR$-vector spaces.
%
Moreover, for every $\phi(x\,;z)\in L$ in $L^{\rm f}$ there is a symbol $\sbar\phi(x^{\rm s}\,;z)$ for a function $F_{|x|}\times M^{|z|}\to\RR$.
%
...
\begin{definition}
  ...
  %`'
  We denote by \emph{$\U^{\rm s}$\/} a saturated elementary extension of $\U^{\rm f}$ of cardinality $\kappa$.
  %
  As all saturated models of cardinality $\kappa$ are isomorphic, we can assume that $\U$ is the home-sort of $\U^{\rm s}$.
  %
  Elements $r\in\U^{\rm f}$ are called \emph{finite samples,} elements in $s\in\U^{\rm s}$ are called \emph{internal samples.}
  %
  Types  $p(x^{\rm s})\in S(\U^{\rm f})$ are called \emph{global samples.}\QED
\end{definition}
...
\begin{notation}
  In general, for any global type $p(x)\in S(\U)$ and formula $\phi(x\,;z)\in L$ we write 
  
  \ceq{\hfill\emph{$\phi(p\,;\U)$}}{=}{\Big\{ b\in\U^{|z|}\ :\ \phi(x\,; b)\in p\Big\}}
  
  This notation intentionally confuses $p$ with any of its realizations in some elementary extension of $\U$.
  %
  Sets of this form are called \emph{externally definable.}
  
  We extend this notation to global samples.
  %
  ...
  %
  In analogy to the terminology above, we may say that $\sbar\phi(p\,;z)$ is an \emph{externally definable\/} real valued function.\QED
\end{notation}

% \begin{definition}\label{def_smooth_global}
%   Let $\phi(x\,;z)\in L$ be given. We say that $p(x^{\rm s})\in S(\U^{\rm f})$ is a \emph{smooth sample\/} if there is an internal sample $s$ such that $\sbar\phi(s\,;b)\approx\sbar\phi(p\,;b)$ for all $b\in\U^{|z|}$.\QED
% \end{definition}

\begin{definition}\label{def_smooth_global}
  We say that $p(x^{\rm s})\in S(\U^{\rm f})$ is a \emph{smooth sample\/} if it is realized in $\U^{\rm s}$.\QED
\end{definition}
...
\begin{fact}
  ...
  %
  Then if $p$ is invariant, it is approximable and definable.
\end{fact}

\begin{proof}
  Let $s\in\U^{\rm s}$ be such that $\sbar\phi(s\,;b)\approx\sbar\phi(p\,;b)$ for all $b\in\U^{|z|}$.
  %
  Assuming \ssf{1} of Definition~\ref{def_genstable}, we prove \ssf{2} and \ssf{3}. 
  
  \ssf{1}$\IMP$\ssf{2.} Let $\mu\in\RR$ be such that $\sbar\phi(p\,;b)\approx\mu\in\RR$.
  %
  Let $b'$ be such that $s\cnonfork_{M^{\rm f}}b'\equiv_{M^{\rm f}}b$.
  %
  By \ssf{1}, $\sbar\phi(s\,;b')\approx\mu$. 
  %
  Therefore there is an $r\in M^{\rm f}$ such that $\sbar\phi(r\,;b')\approx_\epsilon\mu$.
  %
  As $b'\equiv_{M^{\rm f}}b$, we obtain  $\sbar\phi(r\,;b)\approx_\epsilon\mu$.

  \ssf{1}$\IMP$\ssf{3.} The set $\{b:\sbar\phi(p\,;b)<\epsilon\}$ coincides with 
  $\{b:\sbar\phi(s\,;b)<\epsilon\}$, hence is definable in $\U^{\rm s}$.
  %
  As it is invariat over $M^{\rm f}$, it is definable over $M^{\rm f}$.
\end{proof}
...
\begin{lemma}
  ...
  \begin{itemize}
    ...
    \item[2.] $p$ is approximable;
    \item[3.] there are some finite samples $r_1,\dots,r_n\in M^{\rm f}$ such that for every $b\in\U^{|z|}$ we have $\sbar\phi(p\,;b)\approx_\epsilon\sbar\phi(r_i\,;b)$ for some $r_i$ (the samples $r_1,\dots,r_n$ as well as the number $n$ depend on $\epsilon$, but not on $b$);
...
\begin{proof} Let $s\in\U^{\rm s}$ be such that $\sbar\phi(s\,;b)\approx\sbar\phi(p\,;b)$ for all $b\in\U^{|z|}$.

  \ssf{1}$\IMP$\ssf{2}.
  %
  Let $b\in\U^{|z|}$ be arbitrary.
  %
  Let $\mu\in\RR$ be such that $\sbar\phi(p\,;b)\approx\mu\in\RR$.
  %
  Let $b'$ be such that $s\cnonfork_{M^{\rm f}}b'\equiv_{M^{\rm f}}b$.
  %
  By \ssf{1}, $\sbar\phi(s\,;b')\approx\mu$. 
...
\begin{proof}
  ...
  %
  If $b,b'\in\U^{|z|}$ and $b\equiv_Mb'$, then $b'=fb$ for some $f\in\Aut(\U^{\rm s}/M)$.
  %
  ...
  Then $\sbar\phi(s''\,;b)\not\approx_\epsilon\sbar\phi(s\,;b)$.
  %
  As $s''\equiv_Ms$ are both in $\U^{\rm s}$, then $s''=fs$ for some $f\in\Aut(\U^{\rm s}/M)$.
  %
