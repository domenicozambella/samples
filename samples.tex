...
\def\ceq#1#2#3{\parbox[t]{15ex}{$\displaystyle #1$}\medrel{\displaystyle #2}$\displaystyle  #3$}
\def\uuu{{\mathds 1}}
\def\gruuu{{\mathds 1}}
\def\Av{{\rm Av}}
\def\st{{\rm ft}}
...
We put a bar over the symbols that denotes elements of sample-sort.
The \emph{size\/} of a sample $\bar a\in\bar M$ is the length of $\bar a$ as an element of $M^{<\omega}$.
This is denoted by \emph{$\lh\bar a$.}
The same symbol $\bar a$ may  be used for tuples of samples of the same size.
The usual notation \emph{$|\bar a|$\/} is used the length of such tuples, for instance, if $\bar a\in\bar M^n$ then  $|\bar a|=n$.
We write \emph{$\bar a.i$\/} for the $i$-th element of the sample $\bar a$.
Then  $\bar a_i\in M^{|\bar a|}$.
%
The language of this expansion is denoted by $\barL$; it comprises
\begin{itemize}
  \item[1.] all symbols of $L$, these apply to the home-sort;
  \item[2.] $0,1,+,\cdot$ which apply to the integer-sort;
  \item[3.] a ternary relation of sort $\bar M,\omega, M$ that holds if $\bar x.i=y$.
  \item[4.] for every $\phi(x)\in\barL$, where here for clarity $|x|=1$,  there is relation symbol of sort $\bar M,\omega$ that holds for those $\bar a$ and $i<\lh\bar a$ such that $\phi(\bar a.i)$;
  %\item[4'.]  for every $\phi(x\,;y,\bar z)\in\barL$ there is relation symbol of sort $\bar M,\omega$ that holds for those $\bar a$ and $i$ such that $\phi(\bar a.i)$;
\end{itemize}
  
When $\phi(x)$ is the formula $x=x$, then \ssf{4} above implies that $\lh\bar x$ is definable.
Also, from \ssf{4} it easily follows that the function $|\{i<\lh\bar a\, :\, \phi(\bar a.i)\}|$ is definable.
...
To any internal sample $\bar a$ we associate a finitely additive measure on the definable subsets of $\U$.
Define 

\ceq{\hfill\Pr_{\bar a}\big[\phi(\U)\big]}
{=}
{\sup\Big\{r\in\QQ\ :\ r<\Fr_{\bar a}\phi(x)\Big\}}

\begin{definition}
  Let $x$ be a tuple of variables of the home-sort. An \emph{external sample\/} is a global type $p(\bar x)\subseteq \bar L(\U,\omega^*,\bar\U^*)$ such that for every formula $\phi(x)\in \barL$ the set $\{i\in\omega^*:p(\bar x)\proves\phi(\bar x.i)\}$ is definable and bounded.\QED
\end{definition}

For every external sample $p(\bar x)$ there is an $n\in\omega^*$ such that $p(\bar x)\proves\lh\bar x =n$ and the (nonstandard) cardinality of the set $\{i\in\omega^*:p(\bar x)\proves\phi(\bar x.i)\}$ is well-defined.

Therefore, to any external sample $p(\bar x)$ we associate a finitely additive probability measure on the definable subsets of $\U$.
Define 

\ceq{\hfill\Pr_{\bar p}\big[\phi(\U)\big]}
{=}
...
\begin{notation}
  ...
  For any external sample $p(\bar x)$, and every formula $\phi(\bar x\,;z)\in\barL(\U,\omega^*,\bar\U^*)$, and $b\in\U^{|z|}$ we write \emph{$\phi(\bar p\,;b)$} for $p(\bar x)\proves\phi(\bar x; b)$.
  %
  We also write
  ...
  %
  This notation intentionally confuses $p(\bar x)$ with any of its realizations (suggestively denoted by $\bar p$) in some elementary extension of $\barU$.\QED
\end{notation}
...
\begin{definition}\label{def_}
  ...
  \begin{itemize}
    \item[1.]\noindent\emph{invariant\/} if for every $\phi(\bar x;z)\in\barL(\U,\omega^*,\bar\U^*)$ the set $\phi(\bar p\,;\U)$ is invariant over $M$;
    \item[2.]\noindent\emph{finitely satisfiable\/} if for every formula $\phi(\bar x,\;z)\in\barL(\U,\omega^*,\bar\U^*)$ if $\phi(\bar p\,;\U)\neq\0$, then $\phi(\bar a\,;\U)\neq\0$ for some $a\in\bar M$;
    \item[3.]\noindent\emph{definable\/} over $M$ if for every $\phi(\bar x;z)\in\barL(\U,\omega^*,\bar\U^*)$ the set $\phi(p\,;\U)$ is definable over $\bar M$.\QED
  \end{itemize}
\end{definition}

\begin{fact}
  If an external sample $p(\bar x)$ is finitely satisfiable in $M$, then for every $\epsilon>0$ there is an $\bar a\in\bar M$ such that 
...
\begin{definition}\label{def_smooth_global}
  Let $M$ be given.
  %
  An external sample is \emph{smooth\/} if it is invariant and realized in $\barU^*$ (by some internal sample).\QED
\end{definition}
...
\begin{proof}
  Let $s\in\barU^{\rm s*}$ be an internal sample that realizes $p(\bar x)$.
  %
  Let $\phi(\bar x;z)\in L$ be given.
  %
  Firstly, note that $\phi(p\,;\U)=\phi(s\,;\U)$ is definable in $\barU^*$ hence, by invariance, it is definable in $\bar M$.
  ...
  Hence compatness yields some finitely many $r_i\subseteq M^{\rm s}$ such that
