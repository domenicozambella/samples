...
%
We write \emph{$\sbar M \ M^{\rm f}$\/}$=\<M,\RR,S\>$ for the expansion above.
...
%
It expands $L$, the language of ordered rings (on $\RR$), the language of real vector spaces (on $S$).
%
...
\begin{definition}
  ...
  %
  As all saturated models of cardinality $\kappa$ are isomorphic, we can assume that $\U^{\rm f*}=\<\U,\RR^*,\S^*\>$, where $\RR^*$ and $ \S^*$ are suitable saturated extensions of $\RR$, respectively $\S$.
  %
  Elements $\S$ are called \emph{finite samples,} elements in $\S^*$ are called \emph{internal samples.}

  Finally, a \emph{global sample\/} is a type $p(x^{\rm s})\in S(\U^{\rm f})$.\QED
\end{definition}

Every automorphism of $\U^{\rm f*}$ fixes $\S$ (???).
...
\begin{definition}\label{def_} 
  ...
  \begin{itemize}
    \item[1.]\noindent\emph{home-sort invariant\/} if for any $\phi(x^{\rm s}\,;z)\in L(M^{\rm f})$ the set $\phi(p\,;\U)$ is invariant over $M^{\rm f}$;
    \item[2.]\noindent\emph{finitely satisfiable\/} if for every finite subset of $p(x^{\rm s})$ is realized in $M^{\rm f}$;
    \item[3.]\noindent\emph{definable\/} if, for every $\psi(x^{\rm s}\,;z)\in L^{\rm f}$, the set $\psi(p\,;\U)$ is definable over $M^{\rm f}$.\QED
  \end{itemize}
\end{definition}
...
\begin{proof}
  ...
  %
  Let $\phi(x^{\rm s}\,;b)\in p$ for some $\phi(x^{\rm s}\,;z)\in L^{\rm f}$ and $b\in\U^{|z|}$.
  %
  Let $b'$ be such that $s\cnonfork_{M^{\rm f}}b'\equiv_{M^{\rm f}}b$.
  %
  By invariance, $\phi(s\,;b')$.
  %
  Therefore there is an $r\in M^{\rm f}$ such that $\phi(r\,;b')$.
  %
  As $b'\equiv_{M^{\rm f}}b$, we obtain $\phi(r\,;b)$.
  %
  This proves finite satisfiability.
  
  Now, let $\psi(x^{\rm s}\,;z)\in L^{\rm f}$. As $\psi(p\,;\U)=\psi(s\,;\U)$ is definable in $\U^{\rm f*}$.
  %
  By invariance it is definable in $M^{\rm f}$. 
\end{proof}
...
%   %
%   Then for every $\phi(x^{\rm s}\,;z)\in L$ there are some finitely many $r_i\in M^{\rm f}$, say $0\le i<n$, such that for every $b\in\U^{|z|}$ we have $\phi(p\,;b)\iff\phi(r_i\,;b)$ for some $i$.
% \end{fact}
...
\begin{fact}
  ...
  \begin{itemize}
    ...
    \item[2.] $p$ is definable and for every $\phi(x^{\rm s}\,;z)\in L$ there are some finitely many $r_i\in M^{\rm f}$, say $0\le i<n$, such that for every $b\in\U^{|z|}$ we have $\phi(p\,;b)\iff\phi(r_i\,;b)$ for some $i$;
  \end{itemize}
\end{fact}
...
\begin{proof}
  ...
  \ssf{2}$\IMP$\ssf{1} 
  For any given $\phi(x^{\rm s}\,;z)\in L^{\rm f}$ let $\theta(z)\in L(M)$ the formula defining the set $\{b\in\U^{|z|}:\phi(p\,;b)\iff\phi(r_i\,;b)\}$ and let $r_i\in M^{\rm f}$ be as in \ssf{2}.
  %
  Let $p(x^{\rm s})$ cantain the formulas

  \ceq{\,}{\,}{\bigwedge_{i<n}\A z\Big[\theta(z)\wedge\phi(r_i\,;z)\imp\phi(x^{\rm s};z)\Big]}

  as $\phi(x^{\rm s}\,;z)$ ranges in $L^{\rm f}$.
  %
