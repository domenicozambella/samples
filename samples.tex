...
\begin{definition}\label{def_}
  ...
  \begin{itemize}
    ...
    \item[2.]\noindent\emph{finitely satisfiable\/} if every formula in $p(x^{\rm s})$ is satisfied in $\barM$;
...
We say that the global sample $p(x^{\rm s})$ is \emph{strongly finitely satisfiable\/} for every $\phi(x^{\rm s};z)\in L$ there finite set of finite samples $R\subseteq M^{\rm s}$ such that for every $b\in\U^{|z|}$ we have $\phi(p\,;b)\iff\phi(r\,;b)$ for some $r\in R$.\QED
\end{definition}

\begin{lemma}\label{lem_sfs}
  Let $M$ be given.
  %
  For every smooth, invariant global sample $p(x^{\rm s})$ the following hold
  \begin{itemize}
...
\begin{proof}
  ...
  \begin{itemize}
    \item[1.] As $\phi(p\,;\U)=\phi(s\,;\U)$ is definable in $\barU^*$, by invariance it is definable in $\barM$.
    \item[2.] First we prove that $p(x^{\rm s})$ is finitely satisfiable.
    %
    Let $b\in\U^{|z|}$ be arbitrary and let $b'$ be such that $s\cnonfork_{\barM}b'\equiv_{\barM}b$.
    %
