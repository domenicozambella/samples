...
%
We write $a\in s$ for $s(a)$, as this is interpreted as the number of occurreces of $a\in s$.
%
The \emph{size\/} of a sample $s$ is defined to be 
...
%
Moreover, $L^{\rm f}$ has a function for the number of occurrences of a tuple $a$ in a sample $s$, which we denote by \emph{ $a\in s$\/} and a funtion for the size $|s|$.
...
As all saturated models of cardinality $\kappa$ are isomorphic, we can assume that $\U$ is the home-sort of $\U^{\rm f*}$.
 ...
 We write \emph{$\S(s)$\/} for the \emph{support\/} of $s$, that is, the subset of $\U^n$ where $x\in s$ is positive. Note that its cardinality is either finite of $\kappa$. If $s$ is a sample in $\U^{\rm f*}$, we write \emph{$s\subseteq\U^n$\/} to specify that $\S(s)\subseteq\U^n$.
...
\begin{corollary}
  ...
  Let $M$ be any model.
  %
  For every sample $s\subseteq\U^{|x|}$ that is invariant over $M$, and every $\epsilon$, there is a sample $s_\epsilon\subseteq M^{|x|}$ such that $\Av_s\phi(x\,;b)\approx_\epsilon\Av_{s_\epsilon}\,\phi(x\,;b)$ for all $b\in M^{|z|}$.
\end{corollary}
...
%
      ...
      We say that a sample $s\subseteq\U^{|x|}$ is \emph{smooth\/} over $A$ if for any other sample $s'$ such that $\Av_s\,\phi(x)\approx\Av_{s'}\,\phi(x)$ holds for all $\phi(x)\in L(A)$, the same holds for all $\phi(x)\in L(\U)$.\QED
\end{definition}



\begin{definition}[???]
   An sample $s$ is generically stable over $M$ if it is invariant and ???.\QED
\end{definition}
    
\begin{definition}[???]
  A formula $\phi(x\,;z)\in L$ is \emph{distal\/} if there is a formula $\psi(x\,;z_1,\dots z_n)\in L$ such that for every hyperfinite sample $s$ and every $b\in\U^{|x|}$ there are $a_1,\dots,a_n\in s^{|z|}$ such that $\psi(x\,;a_1,\dots,a_n)\iff p(x)$, where $p(x)=\tp_\phi(b/....s)$.\QED
\end{definition}
