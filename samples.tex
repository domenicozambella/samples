...
%
Recall that a multiset is a function $s:M\to\omega$.
%
We write  \emph{ $a\in s$\/} for $s(a)$, and this is interpreted as the number of occurreces of $a\in s$.
%
The \emph{size\/} of a sample $s$ is defined as follows
...
%
Moreover, $L^{\rm f}$ has a function $a\in s$ for the number of occurrences of the tuple $a$ in the sample $s$,  and a funtion $|s|$ for the size of the sample $s$.
...
As all saturated models of cardinality $\kappa$ are isomorphic, we can assume that $\U$ is the home-sort of $\U^{\rm f*}$.
 ...
 We write \emph{$\S(s)$\/} for the \emph{support\/} of $s$, that is, the subset of $\U^n$ where $x\in s$ is positive. Note that its cardinality is either finite of $\kappa$. If $s$ is a sample in $\U^{\rm f*}$, we write \emph{$s\subseteq\U^n$\/} as a shorthand of $\S(s)\subseteq\U^n$.
...
\begin{theorem}
  ...
  Let $M$ be a model.
  %
  For every sample $s\subseteq\U^{|x|}$, and every $\epsilon$, there is a sample $s_\epsilon\subseteq M^{|x|}$ such that $\Av_s\phi(x\,;b)\approx_\epsilon\Av_{s_\epsilon}\,\phi(x\,;b)$ for all $b\in M^{|z|}$.
  
  Moreover, the cardinality of $s_\epsilon$ soley depends on $\epsilon$, namely
...
\begin{remark}[???]
  When the model $M$ in the theorem above is $\omega_1$-saturated, we can claim that there is a (necessarily finite) sample $s_M\subseteq M^{|x|}$ such that $\Av_s\phi(x\,;b)\approx\Av_{s_M}\,\phi(x\,;b)$.\QED
\end{remark}
...
\begin{corollary}
  ...
  Let $M$ be any model.
  %
  For every sample $s\subseteq\U^{|x|}$ that is invariant for $\phi(x\,;z)$ over $M$, and every $\epsilon$, there is a sample $s_\epsilon\subseteq M^{|x|}$ such that $\Av_s\phi(x\,;b)\approx_\epsilon\Av_{s_\epsilon}\,\phi(x\,;b)$ for all $b\in M^{|z|}$.
\end{corollary}

\begin{proof}
  By the theorem above, there is a sample $s_\epsilon\subseteq M^{|x|}$ such that $\Av_s\phi(x\,;b)\approx_\epsilon\Av_{s_\epsilon}\,\phi(x\,;b)$ for all $b\in M^{|z|}$.
  
  Suppose for a contradiction that $\Av_s\phi(x\,;b')\not\approx_\epsilon\Av_{s_\epsilon}\,\phi(x\,;b')$ for some $b'\in\U^{|z|}$.
  %
...
\begin{definition}[???]
  ...
  We say that a sample $s\subseteq\U^{|x|}$ is \emph{smooth\/} for $\phi(x\,;z)$ over $A$ if for any other sample $s'$ such that $\Av_s\,\phi(x\,;b)\approx\Av_{s'}\,\phi(x\,;b)$ for all $b\in M^{|z|}$, the same equivalence holds for all $b\in\U^{|z|}$.\QED
\end{definition}

\begin{definition}[???]
  A sample $s\subseteq\U^{|x|}$ is \emph{finitely satisfiable\/} for $\phi(x\,;z)$ in $M$ if, for every $b\in\U^{|z|}$ such that $\Av_s\phi(x\,;b)\not\approx_\epsilon0$, there is  $a\in M^{|z|}$ such that $\phi(a\,;b)$.
