...
%
Elements of sample-sort in these models are called  \emph{finite samples,} \emph{samples,} and \emph{external samples\/} respectively.\QED
%
...
\begin{fact}
  For every internal sample $s$ following are equivalent
  \begin{itemize}
...
\begin{definition}
  A  external sample $s$ is \emph{finitely satisfiable\/} for $\phi(x\,;z)$ in $M$ if, for every $b\in\U^{|z|}$ such that $\Av_s\phi(x\,;b)\not\approx0$, there is  $a\in M^{|z|}$ such that $\phi(a\,;b)$.\QED
\end{definition}
...
\begin{definition}
  A external sample $s$ is \emph{definable\/} for $\phi(x\,;z)$ over $M$ if, for every $\epsilon$, the set $\{b\in\U^{|z|}:\Av_s\phi(x\,;b)<\epsilon\}$ is definable over $M$.\QED
\end{definition}
...
\begin{definition}
  A formula $\phi(x\,;z)\in L$ is \emph{distal\/} if there is a formula $\psi(x\,;z_1,\dots z_n)\in L$ such that for every finite sample $s\in\U^{\rm f}$ and every $b\in\U^{|x|}$ there are $a_1,\dots,a_n\in \supp(s)$ such that $\psi(x\,;a_1,\dots,a_n)\iff p(x)$, where $p(x)=\tp_\phi(b/\supp(s))$.\QED
\end{definition}
