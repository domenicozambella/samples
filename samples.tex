...
%
The \emph{finite (fractional) sample expansion\/} of $M$ is a many-sorted expansion that has a sort for $M$, which we call the home-sort, a sort for $\RR$, as ordered field, and a sort, called the  \emph{sample-sort,} for the set of functions $f:M\to\RR$ that are almot always $0$.
%
We write \emph{$M^{\rm f}$\/}$=\<M,\RR,F\>$ for the expansion above.

Variables of sample-sort are denoted with symbols $x^{\rm s}, y^{\rm s}$ and variations thereof.
%
The language of $M^{\rm f}$ is denoted by \emph{$L^{\rm f}$.}
%
It expands the laguage of each sort, that is, $L$, the language of orders, the language of real vector spaces.
%
Moreover, for every $\phi(x\,;z,z^{\rm s})\in L^{\rm f}$ there is a symbol for a function 

\ceq{\hfill\sbar\phi(x^{\rm s}\,;z,z^{\rm s})}
{:}
{F^{|x|}\times M^{|z|}\times F^{|z^{\rm s}|}\ \ \longrightarrow\ \ \RR}.
...
\begin{definition}
  Let $\U$ be a monster model of cardinality $\kappa>|L|$ and let $\U^{\rm f}=\<\U,\RR,\F\>$ be the corresponding expansion.
  %
  We denote by \emph{$\U^{\rm f*}$\/} a saturated elementary extension of $\U^{\rm f}$ of cardinality $\kappa$.
  %
  As all saturated models of cardinality $\kappa$ are isomorphic, we can assume that $\U$ is the home-sort of $\U^{\rm f*}$.
  %
  Elements $r\in\U^{\rm f}$ are called \emph{finite samples,} elements in $s\in\U^{\rm f*}$ are called \emph{internal samples.}

  Finally, a \emph{global sample\/} is a type $p(x^{\rm s})\in S(\U^{\rm f})$.\QED
\end{definition}

Note that $\U^{\rm f}$ is not saturated but it is homogeneous.


\begin{definition}\label{def_smooth_global}
  A global sample $p(x^{\rm s})$ is \emph{smooth\/} if it is realized in $\U^{\rm f*}$.\QED
\end{definition}
...
\begin{definition}\label{def_} 
  ...
  \begin{itemize}
    \item[1.]\noindent\emph{invariant\/} if for every formula $\phi(x^{\rm s}\,;z,z
    ^{\rm s})$ the set $\phi(p\,;\U,\F)$ is invariant over $M^{\rm f}$;
    \item[2.]\noindent\emph{finitely satisfiable\/} if for every finite subset of $p(x^{\rm s})$ is realized in  $M^{\rm f}$.
    \item[3.]\noindent\emph{definable\/} if, for every $\psi(x^{\rm s}\,;z,z^{\rm s})\in L^{\rm f}$, the set $\psi(p\,;\U,\F)$ is definable over $M^{\rm f}$.\QED
  \end{itemize}
\end{definition}
...
\begin{notation}
  For any global type $p(x^{\rm s})\in S(\U)$ and formula $\psi(x^{\rm s}\,;z)\in L^{\rm f}$ we write 
  
  \ceq{\hfill\emph{$\psi(p\,;\U)$}}{=}{\Big\{ b\in\U^{|z|}\ :\ \psi(x^{\rm s}\,; b)\in p\Big\}}
  
  (the same notation applies with $x$ for $x^{\rm s}$). This notation intentionally confuses $p$ with any of its realizations in some elementary extension of $\U^{\rm f}$.
  %
  Sets of this form are called \emph{externally definable.}
  %
  The type $p(x^{\rm s})$ may be identified with a collection of externally definable sets.
  
  We extend this notation to functions.
  %
  If $\phi(x\,;z)\in L$ and $p(x^{\rm s})$ is a global sample, we write $\sbar\phi(p\,;b)$ for the unique $\mu\in\RR$ such that $p(x^{\rm s})\proves \bar\phi(x^{\rm s}\,;b)\approx\mu$.
  %
  In analogy to the terminology above, we may say that $\sbar\phi(p\,;z)$ is an \emph{externally definable\/} real valued function.
  %
  A global sample may be identified with a family of externally definable functions.
  \QED
\end{notation}

Explicitely, \ssf{2} above means that if $\sbar\psi_i(x^{\rm s})\approx_\epsilon\mu_i$ are some finitely many formulas in $p$, then there is an $r\in M^{\rm f}$ such that $\sbar\psi_i(r)\approx_\epsilon\mu_i$ for all $i$.
%
We may also rephrase this by saying that for every finitely many $\psi_i(x)\in L(\U)$ there is an $r\in M^{\rm f}$ such that $\sbar\psi_i(r)\approx_\epsilon\sbar\psi_i(p)$ for all $i$.
...
\begin{fact}
  ...
  %
  Then, if $p$ is finitely satisfiable , it is is invariant.
\end{fact}
...
\begin{proof}
  ...
  %
  Let $\sbar\phi_i(x^{\rm s}\,;b)\approx_\epsilon\mu_i$ be some finitely many formulas in $p$.
  %
  ...
  Now, let $\psi(x^{\rm s}\,;z)\in L^{\rm f}$. As $\psi(p\,;\U)=\psi(s\,;\U)$ is definable in $\U^{\rm f*}$. By invariance it is definable in $M^{\rm f}$. 
\end{proof}
...
\begin{fact}
  ...
  Let $p(x^{\rm s})$ be a smooth sample.
  %
  Then for every $\phi(x\,;z)\in L$ and every $\epsilon$ there are some finitely many $r_i\in M^{\rm f}$, say $0\le i<n$, such that for every $b\in\U^{|z|}$ we have $\sbar\phi(p\,;b)\approx_\epsilon\sbar\phi(r_i\,;b)$ for some $i$.
\end{fact}
...
\begin{fact}
  ...
  \begin{itemize}
    \item[1.] $p$ is smooth;
    \item[2.] $p$ is definable and for every $\phi(x\,;z)\in L$ and every $\epsilon$ there are some finitely many $r_i\in M^{\rm f}$, say $0\le i<n$, such that for every $b\in\U^{|z|}$ we have $\sbar\phi(p\,;b)\approx_\epsilon\sbar\phi(r_i\,;b)$ for some $i$.
    \item[3.] $p$ is definable and for every $\phi(x)\in L(\U)$ and every $\epsilon$ there is an $r\in M^{\rm f}$ such that $\sbar\phi(p)\approx_\epsilon\sbar\phi(r)$.
  \end{itemize}
\end{fact}
...
\begin{proof}
  ...
  \ssf{2}$\IMP$\ssf{1} For every $r\in M^{\rm f}$ and $\phi(x\,;z)\in L$ let $\theta_{r,\phi}(z)\in $ the formula defining the set $\{b:\sbar\phi(p\,;b)\not\approx_\epsilon\sbar\phi(r\,;b)\}$. By assumption, the following type is finitely consistent

  \ceq{\hfill p(x^{\rm s})}{=}{\Big\{\A z\Big[\theta_{r,\phi}(z)\imp\sbar\phi(x^{\rm s}\,;z)\not\approx_\epsilon\sbar\phi(r\,;z)\Big]\ :\ r\in M^{\rm f}\Big\}}
\end{proof}
...
\begin{proof}
  ...
  %
  We claim that there is an $r\in M^{\rm f}$ such that $\sbar\phi_i(p)\approx_{3\epsilon}\sbar\phi_i(r)$ for every $i$.
  ...
  %
  By elementarity there is an $r\in M^{\rm f}$ such that $\theta_{i,1}(r)\approx_\epsilon\theta_{i,1}(p)$ and $\theta_{i,2}(r)\approx_\epsilon\theta_{i,2}(p)$.
  %
...
%Clearly, if $s$ is an internal sample, then $p(x^{\rm s})=\tp(s/\U^{\rm f})$ is a smooth sample w.r.t.\@ any given formula $\phi(x\,;z)\in L$.
...
\begin{definition}\label{def_genstable}
  ...
  A global sample $p(x^{\rm s})$ is
  \begin{itemize}
    \item[1.]\noindent \emph{invariant\/} if $\bar\phi(p\,;b)\approx\bar\phi(p\,;b')$ for every $b\equiv_{M^{\rm f}}b'$;
    \item[2.]\noindent (pointwise) \emph{approximable\/} if, for every $b\in\U^{|z|}$ and every $\epsilon$, there is an $r\in M^{\rm f}$ such that $\sbar\phi(p\,;b)\approx_\epsilon\sbar\phi(r\,;b)$;
    \item[3.]\noindent \emph{definable\/} if, for every $\epsilon$, the set $\{b\in\U^{|z|}:\sbar\phi(p\,;b)<\epsilon\}$ is definable over $M^{\rm f}$.\QED 
  \end{itemize}
\end{definition}
...
\section{Old stuff/garbage}
 ...
 %
 If $s$ is a smooth sample  \emph{$\supp(s)$\/} is defined to be  \emph{$\supp(p)$\/} for $p(x^{\rm s})=\tp(s/\U^{\rm f})$. 
...
\begin{definition}\label{def_genstable}
 ...
 An external sample $s$ is 
  \begin{itemize}
    ...
    \item [3.]\noindent  \emph{finitely satisfiable\/} if, for all $b\in\U^{|z|}$ there is an $r\in M^{\rm f}$ such that  $\sbar\phi(s\,;b)\approx_\epsilon\sbar\phi(r\,;b)$;
