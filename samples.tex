...
The \emph{finite (fractional) expansion\/} of a structure $M$ is a many-sorted expansion that has a sort for $M$, which we call the home-sort, a sort for the natural numbers, and a sort for the finite fractional multisets of elements of $M^n$, for any $n<\omega$.
%
...
As all saturated models of cardinality $\kappa$ are isomorphic, we can assume that $\U$ is the home-sort of $\U^{\rm f*}$.
 ...
 We write \emph{$\supp(s)$\/} for the \emph{support\/} of $s$, that is, the subset of $\U^n$ where $x\in s$ is positive. Note that its cardinality is either finite of $\kappa$. If $s$ is a sample in $\U^{\rm f*}$, we write \emph{$s\subseteq\U^n$\/} as a shorthand of $\supp(s)\subseteq\U^n$.
...
\begin{proof}
  Let $M^{\rm f*}\preceq\U^{\rm f*}$ be a sufficiently saturated model containing $B$. Define  for every $\epsilon$
...
By saturation there is an $s'\in M^{\rm f*}$ that realises all these types. As $s'\subseteq M^{|x|}$ then $\supp(s')$ is finite. Clearly, we can assume that $a\in s'$ is standard for all $a$. So the claim follows.
\end{proof}
