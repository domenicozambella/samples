...
%
The \emph{fractional multiset expansion\/} of $M$ is a many-sorted expansion that has a sort for $M$, which we call the home-sort, a sort for $\RR$, and a \emph{sample-sort\/} for the set of functions $s:M\to\RR$ that are almot always $0$.
%
We write \emph{$\sbar M$\/}$=\<M,\RR,M^{\rm s}\>$ for the expansion above.

Below $x,y,z$ are always tuples of variables of home-sort, $x^{\rm s},y^{\rm s},z^{\rm s}$ are tuples of sample-sort of the same the length of $x,y$, respectively $z$.

The language of $\sbar M$ is denoted by \emph{$\sbar L$.}
%
It expands $L$, the language of ordered rings (on $\RR$), the language of real vector spaces (on $M^{\rm s}$).
%
Moreover, for every $\phi(x\,;z)\in L$ there is a symbol for a function 

\ceq{\hfill\sbar\phi(x^{\rm s};z)}
{:}
{S^{|x|}\times M^{|z|}\ \ \longrightarrow\ \ \RR}.

%For the moment we only require that $\sbar\phi(f\,;b)\neq0$ then $\E x\,\phi(x\,;b)$.
...
\begin{definition}
  Let $\U$ be a monster model of cardinality $\kappa>|L|$ and let $\sbar\U=\<\U,\RR,\U^{\rm s}\>$ be the corresponding expansion.
  %
  We denote by \emph{$\sbar\U^*$\/} a saturated elementary extension of $\sbar\U$ of cardinality $\kappa$.
  %
  As all saturated models of cardinality $\kappa$ are isomorphic, we can assume that $\sbar\U^*=\<\U,\,\RR^*,\U^{\rm s*}\>$, where $\RR^*$ and $\U^{\rm s*}$ are suitable saturated extensions of $\RR$, respectively $\U^{\rm s}$.
  %
  Elements $\U^{\rm s}$ are called \emph{finite samples,} elements in $\U^{\rm s*}$ are called \emph{internal samples.}

  Finally, a \emph{global sample\/} is a type $p(x^{\rm s})\in S(\U,\sbar M)$, where $M$ is a model which will be clear from the context.\QED
\end{definition}


\begin{notation}
  For any global type $p(x^{\rm s})\in S(\U,\sbar M)$, formula $\psi(x^{\rm s}\,;z)\in\sbar L(\sbar M)$, and $b\in\U^{|z|}$ we write \emph{$\psi(p\,;b)$} for $\psi(x^{\rm s}\,; b)\in p$.
  %
  We also write
  
  \ceq{\hfill\emph{$\psi(p\,;\U)$}}{=}{\Big\{ b\in\U^{|z|}\ :\ \psi(p\,;b)\Big\}}
  
  (the same notation applies with $x$ for $x^{\rm s}$). This notation intentionally confuses $p$ with any of its realizations in some elementary extension of $\sbar\U$.
  %
  Sets of this form are called \emph{externally definable.}\QED
\end{notation}

\begin{definition}\label{def_smooth_global}
  A global sample $p(x^{\rm s})$ is \emph{smooth\/} if it is realized in $\sbar\U^*$.\QED
\end{definition}
...
\begin{definition}\label{def_} 
  Let $M$ be given.
  %
  A global sample $p(x^{\rm s})$ is
  \begin{itemize}
    \item[1.]\noindent\emph{home-sort invariant\/} if for any $\phi(x^{\rm s}\,;z)\in\sbar L(\sbar M)$ the set $\phi(p\,;\U)$ is invariant over $\sbar M$;
    \item[2.]\noindent\emph{finitely satisfiable\/} if for every finite subset of $p(x^{\rm s})$ is realized in $\sbar M$;
    \item[3.]\noindent\emph{definable\/} if, for every $\psi(x^{\rm s}\,;z)\in\sbar L(\sbar M)$, the set $\psi(p\,;\U)$ is definable over $\sbar M$.\QED
  \end{itemize}
\end{definition}
...
\begin{fact}
  Let $M$ be given.
  %
  Let $p(x^{\rm s})$ be a global sample.  
  %
  Then, if $p$ is finitely satisfiable, it is is invariant.\QED
\end{fact}
...
\begin{fact}\label{fact_smooth=>fs}
  ...
  %
  Then, if $p$ is invariant, it is also finitely satisfiable and definable.
\end{fact}

\begin{proof}
  Let $s$ be an internal sample that realizes $p(x^{\rm s})$.
  %
  Let $\phi(x^{\rm s}\,;b)\in p$ for some $\phi(x^{\rm s}\,;z)\in \sbar L(\sbar M)$ and $b\in\U^{|z|}$.
  %
  Let $b'$ be such that $s\cnonfork_{\sbar M}b'\equiv_{\sbar M}b$.
  %
  By invariance, $\phi(s\,;b')$.
  %
  Therefore there is an $r\in M^{\rm s}$ such that $\phi(r\,;b')$.
  %
  As $b'\equiv_{\sbar M}b$, we obtain $\phi(r\,;b)$.
  %
  This proves finite satisfiability.
  
  Now, let $\psi(x^{\rm s}\,;z)\in \sbar L(\sbar M)$. As $\psi(p\,;\U)=\psi(s\,;\U)$ is definable in $\sbar\U^*$.
  %
  By invariance it is definable in $\sbar M$. 
\end{proof}

% \begin{fact}
%   Let $M$ be given.
%   %
%   Let $p(x^{\rm s})$ be a smooth invariant sample.
%   %
%   Then for every $\phi(x^{\rm s}\,;z)\in L$ there are some finitely many $r_i\in \sbar M$, say $0\le i<n$, such that for every $b\in\U^{|z|}$ we have $\phi(p\,;b)\iff\phi(r_i\,;b)$ for some $i$.
% \end{fact}
...
%   \ceq{\hfill p(z)}{=}{\Big\{\phi(s\,;z)\niff\phi(r\,;z)\ :\ r\in \sbar M\Big\}}
  
%   is inconsistent by Fact~\ref{fact_smooth=>fs}.
%   %
%   Hence compatness yields the required $r_i\in \sbar M$.
% \end{proof}

\begin{fact}
  Let $M$ be given.
  %
  For every global sample $p(x^{\rm s})$ the following are equivalent
  \begin{itemize}
    \item[1.] $p$ is smooth and invariant;
    \item[2.] $p$ is definable and for every $\phi(x^{\rm s}\,;z)\in L$ there are some finitely many $r_i\in \sbar M$, say $0\le i<n$, such that for every $b\in\U^{|z|}$ we have $\phi(p\,;b)\iff\phi(r_i\,;b)$ for some $i$;
    \item[3.] $p$ is definable and finitely satisfiable.
  \end{itemize}
\end{fact}

\begin{proof}
  \ssf{1}$\IMP$\ssf{2} definability has been proved above.
  %
  Let $s$ be an internal sample that realizes $p(x^{\rm s})$.
  %
  Let $\phi(x\,;z)\in L$ and $\epsilon$ be fixed.
  ...
  \ceq{\hfill q(z)}{=}{\Big\{\phi(s\,;z)\niff\phi(r\,;z)\ :\ r\in \sbar M\Big\}}
  
  is inconsistent by Fact~\ref{fact_smooth=>fs}.
  %
  Hence compatness yields the required $r_i\in\sbar M$.

  \ssf{2}$\IMP$\ssf{3} Is clear.

  \ssf{3}$\IMP$\ssf{1} 
  Invariance has been proved above.
  %
  For any $\phi(x^{\rm s}\,;z)\in \sbar L(\sbar M)$ and $r\in M^{\rm s}$ there is a formula $\theta(z)\in L(M)$ defining the set $\{b\in\U^{|z|}:\phi(p\,;b)\iff\phi(r\,;b)\}$.
  %
  Let $q(x^{\rm s})$ contain the formulas

  \ceq{\,}{\,}{\A z\Big[\theta(z)\wedge\phi(r\,;z)\imp\phi(x^{\rm s};z)\Big].}

  It is clear that $q(x^{\rm s})$ is equivalent to $p(x^{\rm s})$.  
  %
  As $q(x^{\rm s})$ is a type over $\sbar M$, it is realized in $\sbar\U^*$.
\end{proof}


\section{Less abstract samples}
...
\begin{proof}
  ...
  %
  We claim that there is an $r\in \sbar M$ such that $\sbar\phi_i(p)\approx_{3\epsilon}\sbar\phi_i(r)$ for every $i$.
  ...
  %
  By elementarity there is an $r\in \sbar M$ such that $\theta_{i,1}(r)\approx_\epsilon\theta_{i,1}(p)$ and $\theta_{i,2}(r)\approx_\epsilon\theta_{i,2}(p)$.
  %
...
\begin{lemma}
  ...
  \begin{itemize}
    \item[1.] $p$ is smooth and invariant;
    \item[2.] for every finite $B\subseteq\U^{|z|}$ and every $\epsilon$, there is an $r\in \sbar M$ such that $\sbar\phi(p\,;b)\approx_\epsilon\sbar\phi(r\,;b)$ for all $b\in B$;
    \item[2.] for every $\phi_i(x\,;z)\in L$, every $b_i\in\U^{|z|}$, and every $\epsilon$, there is an $r\in \sbar M$ such that $\sbar\phi_i(p\,;b_i)\approx_\epsilon\sbar\phi_i(r\,;b_i)$, for every $0\le i<n$;
    \item[2.] for every $\psi_0(x),\dots,\psi_{n-1}(x)\in L(\U)$ and every $\epsilon$, there is an $r\in \sbar M$ such that $\sbar\psi_i(p)\approx_\epsilon\sbar\psi_i(r)$, for every $i<n$;
...
\begin{proof} Let $s$ be an internal sample that realizes $p(x^{\rm s})$.
  ...
  %
  Let $\bar b'$ be such that $s\cnonfork_{\sbar M}\bar b'\equiv_{\sbar M}\bar b$.
  %
  By invariance, $\sbar\phi(s\,;b'_i)\approx\mu_i$. 
  %
  Therefore there is an $r\in \sbar M$ such that $\sbar\phi(r\,;b'_i)\approx_\epsilon\mu_i$ for every $i<n$.
  %
  As $\bar b'\equiv_{\sbar M}\bar b$, we obtain  $\sbar\phi(r\,;b_i)\approx_\epsilon\mu_i$.
  ...
  %
  Therefore there is an $r\in \sbar M$ such that $\sbar\phi_i(r\,;b'_i)\approx_\epsilon\mu_i$ for every $i<n$.
  %
  As $\bar b'\equiv_{\sbar M}\bar b$, we obtain  $\sbar\phi_i(r\,;b_i)\approx_\epsilon\mu_i$.
  ...
  \ceq{\hfill p(z)}{=}{\Big\{\sbar\phi(s\,;z)\not\approx_\epsilon\sbar\phi(r\,;z)\ :\ r\in \sbar M\Big\}}
  
  is inconsistent by \ssf{2}.
  %
  Hence compatness yields the required $r_1,\dots,r_n\in \sbar M$.
  ...
  %
  Pick some $b\equiv_{\sbar M}b'$ and some $\epsilon$.
  %
  Let $r,r'\in \sbar M$ be such that $\sbar\phi(p\,;b)\approx_\epsilon\sbar\phi(r\,;b)$ and $\sbar\phi(p\,;b')\not\approx_\epsilon\sbar\phi(r';b')$.
  %
  As $\sbar\phi(r\,;b)\approx\sbar\phi(r';b')$ follows from  $b\equiv_{\sbar M}b'$, we obtain $\sbar\phi(p\,;b)\approx_\epsilon\sbar\phi(p\,;b')$.
...
%Clearly, if $s$ is an internal sample, then $p(x^{\rm s})=\tp(s/\sbar\U)$ is a smooth sample w.r.t.\@ any given formula $\phi(x\,;z)\in L$.
...
\begin{definition}\label{def_genstable}
  ...
  A global sample $p(x^{\rm s})$ is
  \begin{itemize}
    \item[1.]\noindent \emph{invariant\/} if $\bar\phi(p\,;b)\approx\bar\phi(p\,;b')$ for every $b\equiv_{\sbar M}b'$;
    \item[2.]\noindent (pointwise) \emph{approximable\/} if, for every $b\in\U^{|z|}$ and every $\epsilon$, there is an $r\in \sbar M$ such that $\sbar\phi(p\,;b)\approx_\epsilon\sbar\phi(r\,;b)$;
    \item[3.]\noindent \emph{definable\/} if, for every $\epsilon$, the set $\{b\in\U^{|z|}:\sbar\phi(p\,;b)<\epsilon\}$ is definable over $\sbar M$.\QED 
  \end{itemize}
\end{definition}
...
\section{Old stuff/garbage}
 ...
 %
 If $s$ is a smooth sample  \emph{$\supp(s)$\/} is defined to be  \emph{$\supp(p)$\/} for $p(x^{\rm s})=\tp(s/\sbar\U)$. 
...
\begin{definition}\label{def_genstable}
 ...
 An external sample $s$ is 
  \begin{itemize}
    ...
    % \item [3.]\noindent  \emph{satisfiable\/} if, for all $b\in\U^{|z|}$, if $\sbar\phi(s\,;b)>\epsilon$, then  $\phi(r\,;b)>\epsilon$ for some $r\in \sbar M$.
    \item [3.]\noindent  \emph{finitely satisfiable\/} if, for all $b\in\U^{|z|}$ there is an $r\in \sbar M$ such that  $\sbar\phi(s\,;b)\approx_\epsilon\sbar\phi(r\,;b)$;
...
\begin{proof}
  ...
  %
  If $b,b'\in\U^{|z|}$ and $b\equiv_Mb'$, then $b'=fb$ for some $f\in\Aut(\sbar\U^*/M)$.
  %
  ...
  Then $\sbar\phi(s''\,;b)\not\approx_\epsilon\sbar\phi(s\,;b)$.
  %
  As $s''\equiv_Ms$ are both in $\sbar\U^*$, then $s''=fs$ for some $f\in\Aut(\sbar\U^*/M)$.
  %
...
\begin{definition}\label{def_approximable_sample}
  ...
  %
  We say that $s$ is \emph{(uniformly) approximable\/} on $\B$ if for every $\epsilon$ there is a finite sample $r\in \sbar M$ such that $\sbar\phi(s,b)\approx_\epsilon\sbar\phi(r,b)$ for every $b\in\B$. (The sample $r$ depends on $\epsilon$, not on $b$.)\QED
\end{definition}
...
\begin{proof}
  Let $r\in \sbar M$ be as in Definition~\ref{def_approximable_sample}.
  %
...
\begin{proof}
  By Theorem~\ref{thm_VC}, there is an $r\in \sbar M$ such that $\sbar\phi(s\,;b)\approx_\epsilon\sbar\phi(r\,;b)$ for all $b\in M^{|z|}$.
...
For every finite sample $r\in \sbar M$ and every $\mu\in\RR$ there is a formula $\psi(z)\in L(\supp r)$ such that $\sbar\phi(r\,;z)=\mu \iff \psi(z)$. The formula $\psi(z)$ depends on $r$.
...
There is a formula $\psi(x_1,\dots,x_n\,;z)$ such that for every $r\in \sbar M$ 
