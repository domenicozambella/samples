...
%
It will be defined in the next section as it is not relevant for the moment.
...
\begin{definition}
  ...
  Finally, a \emph{global sample\/} is a type $p(x^{\rm s})\in S(\U,\barM)$, where $M$ is a model which will be clear from the context.\QED
\end{definition}


\begin{notation}
  For any global type $p(x^{\rm s})\in S(\U,\barM)$, formula $\psi(x^{\rm s};z)\in\barL(\barM)$, and $b\in\U^{|z|}$ we write \emph{$\psi(p\,;b)$} for $\psi(x^{\rm s}; b)\in p$.
  %
  We also write
...
\begin{definition}\label{def_smooth_global}
  A global sample $p(x^{\rm s})$ is \emph{smooth\/} if it is realized in $\barU^*$.\QED
\end{definition}
...
\begin{definition}\label{def_}
  ...
  \begin{itemize}
    \item[1.]\noindent\emph{invariant\/} if for any $\phi(x^{\rm s};z)\in\barL(\barM)$ the set $\phi(p\,;\U)$ is invariant over $\barM$;
    \item[2.]\noindent\emph{finitely satisfiable\/} if for every formula in $p(x^{\rm s})$ is satisfied in $\barM$;
    \item[3.]\noindent\emph{definable\/} if, for every $\psi(x^{\rm s};z)\in\barL(\barM)$, the set $\psi(p\,;\U)$ is definable over $\barM$.\QED
  \end{itemize}
\end{definition}
...
The following proposition is an intermediate step towards the subsequent lemma.

\begin{proposition}\label{fact_smooth=>fs}
  Let $M$ be given.
  %
  Let $p(x^{\rm s})$ be an invariant smooth sample.
  %
  Then $p$ is finitely satisfiable and definable.
\end{proposition}

\begin{proof}
  Let $s\in\barU^{\rm s*}$ be an internal sample that realizes $p(x^{\rm s})$.
  %
  Let $\phi(x^{\rm s};b)\in p$ for some $\phi(x^{\rm s};z)\in \barL(\barM)$ and $b\in\U^{|z|}$.
  %
  Let $b'$ be such that $s\cnonfork_{\barM}b'\equiv_{\barM}b$.
  %
  By invariance, $\phi(s\,;b')$.
  %
  Therefore there is an $r\in M^{\rm s}$ such that $\phi(r\,;b')$.
  %
  As $b'\equiv_{\barM}b$, we obtain $\phi(r\,;b)$.
  %
  This proves finite satisfiability.

  Now, let $\psi(x^{\rm s};z)\in \barL(\barM)$.
  %
  As $\psi(p\,;\U)=\psi(s\,;\U)$ is definable in $\barU^*$, by invariance it is definable in $\barM$.
\end{proof}

Note that a global sample $p(x^{\rm s})$ is finitely satisfiable if for every $\phi(x^{\rm s};z)\in\barL(\barM)$ and every $b\in\U^{|z|}$ there is an $r\in M^{\rm s}$ such that $\phi(p\,;b)\iff\phi(r\,;b)$.
...
\begin{lemma}
  ...
  \begin{itemize}
    ...
    \item[2.] $p$ is definable and for every $\phi(x^{\rm s};z)\in L$ there are some finitely many $r_i\in M^{\rm s}$, say $0\le i<n$, such that for every $b\in\U^{|z|}$ we have $\phi(p\,;b)\iff\phi(r_i\,;b)$ for some $i$.
  \end{itemize}
\end{lemma}

\begin{proof}
  \ssf{1}$\IMP$\ssf{2} definability has been proved in Fact~\ref{fact_smooth=>fs}.
  %
  Let $s\in\barU^{\rm s*}$ be an internal sample that realizes $p(x^{\rm s})$.
  %
  Let $\phi(x^{\rm s};z)\in\barL(\barM)$.
  %
  The type

  \ceq{\hfill q(z)}{=}{\Big\{\phi(s\,;z)\niff\phi(r\,;z)\ :\ r\in \barM\Big\}}

  is inconsistent by Fact~\ref{fact_smooth=>fs}.
  %
  Hence compatness yields the required  $r_i\in M^{\rm s}$.

  \ssf{2}$\IMP$\ssf{1}
  As \ssf{2} implies that $p$ is finitely satisfiable, invariance is clear.
  %
  For a given $\phi(x^{\rm s};z)\in \barL(\barM)$ let $r_i\in M^{\rm s}$ be as in \ssf{2}.
  %
  By the definability of $p$ there are some formulas $\theta_i(z)\in L(M)$ that define the sets $\{b\in\U^{|z|}:\phi(p\,;b)\iff\phi(r_i\,;b)\}$.
  %
  Note that by \ssf{2}

  \ceq{\,}{\,}{\A z\ \bigvee_{i<n}\theta_i(z).}

  Let $q(x^{\rm s})$ be the type containing the formulas

  \ceq{\,}{\,}{\bigwedge_{i<n}\A z\Big[\theta_i(z)\wedge\phi(r_i\,;z)\imp\phi(x^{\rm s};z)\Big].}

  as $\phi(x^{\rm s};z)$ range as above.
  %
  This type is consistent and in fact equivalent to $p(x^{\rm s})$.
  %
  As $q(x^{\rm s})$ is a type over $\barM$, it is realized in $\barU^*$.
\end{proof}
...
\begin{itemize}
  ...
  \item[2.] the language of ordered rings, that applies to $\RR$;
...
We assume that for every $\phi(x),\psi(x)\in\barL(\barU)$ if $\phi(x)\imp\psi(x)$ then $\sbar\phi(x^{\rm s})\le\sbar\psi(x^{\rm s})$.
