...
\begin{proof}
  ...
  %
  Then $\supp(s)$ is finite, say $\supp(s)=\{a_1,\dots,a_n\}$.
  %
...
\begin{corollary}
  For every formula $\phi(x\,;z)\in L$, every sample $s\subseteq\U^{|x|}$ and every sequence of indiscernibles $\<b_i:i<\omega\>$ such that $\Av_s\phi(x\,;b_i)>\epsilon$ for every $i<\omega$, the type $\{\phi(x\,;b_i):i<\omega\}$ is consistent.\QED
\end{corollary}
...
\begin{corollary}
  ...
  %
  There is no infinite set $C$ such that $\Av_s\big[\psi(x\,;c)\niff\psi(x\,;c')\big]>\epsilon$ for all $c,c'\in C$.
\end{corollary}

\begin{proof}
  Apply Lemma~\ref{lem_cons} to the folmula $\phi(x\,;z,z') = [\psi(x\,;z)\niff\psi(x\,;z')]$ and the set $B=C^2$.
  %
  Without loss of generality, we may assume that the sets $B_i$ given by the lemma form a partition of the complete graph on $C$.
  %
  By the Ramsey theorem there is an infinite $A\subseteq C$ such that $\{\psi(x\,;a)\niff\psi(x\,;a') : a,a'\in A, a\neq a'\}$ is consistent. Clearly this is a contradiction as soon as $|A|>2$.
\end{proof}
