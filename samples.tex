...
%
The \emph{finite (fractional) sample expansion\/} of $M$ is a many-sorted expansion that has a sort for $M$, which we call the home-sort, a sort for $\RR$ as ordered field, and a \emph{sample-sort\/} for the set of functions $s:M\to\RR$ that are almot always $0$.
%
We write \emph{$M^{\rm f}$\/}$=\<M,\RR,S\>$ for the expansion above.

Below $x$ is always a tuple of variables of the home sort, $x^{\rm s}$ a tuple of variables of the sample-sort, and $z$ a tuple of variables of the mixed sort.
...
%
Moreover, for every $\phi(x\,;z)\in L^{\rm f}$ there is a symbol for a function 
...
\begin{definition}
  Let $\U$ be a monster model of cardinality $\kappa>|L|$ and let $\U^{\rm f}=\<\U,\RR,\S\>$ be the corresponding expansion.
  %
...
\begin{notation}
  For any global type $p(x^{\rm s})\in S(\U)$ and formula $\psi(x^{\rm s}\,;z^{\rm f})\in L^{\rm f}$ we write 
  
  \ceq{\hfill\emph{$\psi(p\,;\U^z)$}}{=}{\Big\{ b\in\U^{z^{\rm f}}\ :\ \psi(x^{\rm s}\,; b)\in p\Big\}}
...
\begin{definition}\label{def_} 
  ...
  \begin{itemize}
    \item[1.]\noindent\emph{invariant\/} if for every formula $\phi(x^{\rm s}\,;z
    ^{\rm f})$ the set $\phi(p\,;\U^{\rm f})$ is invariant over $M^{\rm f}$;
    \item[2.]\noindent\emph{finitely satisfiable\/} if for every finite subset of $p(x^{\rm s})$ is realized in  $M^{\rm f}$.
    \item[3.]\noindent\emph{definable\/} if, for every $\psi(x^{\rm s}\,;z^{\rm f})\in L^{\rm f}$, the set $\psi(p\,;\U^{\rm f})$ is definable over $M^{\rm f}$.\QED
  \end{itemize}
\end{definition}
...
\begin{proof}
  Let $a^{\rm s}$ be an internal sample that realizes $p(x^{\rm s})$.
  %
  Let $\phi_i(x^{\rm s}\,;z^{\rm f})\in L^{\rm f}$ and assume $\phi_i(x^{\rm s}\,;b^{\rm f})\in p$.
  %
  Let $b_0^{\rm f}$ be such that $s\cnonfork_{M^{\rm f}}b_0^{\rm f}\equiv_{M^{\rm f}}b^{\rm f}$.
  %
